%% AAH  St  并行
% L is even
function [Se_all, VarSl_all, index] = fn_AAH_St(L, gam, Dis_str, aver_num, tnum, phase)
%% parameter
% L = 800;        %system size
...
%%
Se_all = zeros(tnum,aver_num);
VarSl_all = zeros(tnum,aver_num);
parfor jj3=1:aver_num   % average evolution trajectory
    ...
    for jj1=1:tnum    % evolution time
        ...
        Se_all(jj1,jj3) = -1*sum(SS);
        
        %% fluctuation VarSl
        U_nn = U0(:,N+1:L);
        D_nn = U_nn * U_nn';
        vv2 = D0(1:Lsub,1:Lsub) .*D_nn(1:Lsub,1:Lsub);
        VarSl_all(jj1,jj3) = sum(vv2,'all');  %tnum*aver_sum
