%% 有限元矩阵 坐标矩阵x_Q (在有限元基底下)
% 参数上与 fn_FEM_matrix(m1,m2) 一致

function [x_Q, x_Q_odd, x_Q_even, p,t] = fn_FEM_x_Q(m1,m2)
%% 参数
% tic
L=1;
...
%pdemesh(p,e,t)    %显示网格
%% 组装矩阵
x_Q=fn_Q(p,t);  % x_Q=fn_Q(p,t); QQ=fn_Q(p,t); x_Q=QQ
...
%% even parity 
num_max = size(x_Q,1);
num_mid = round(size(x_Q,1)/2);
x_Q_even = x_Q(1:num_mid, 1:num_mid) + fliplr(x_Q(1:num_mid, num_max+1-num_mid:num_max))...
       + flipud(x_Q(num_max+1-num_mid:num_max, 1:num_mid))...
       + rot90(x_Q(num_max+1-num_mid: num_max, num_max+1-num_mid: num_max),2);

   
%% odd parity
x_Q_odd = x_Q(1:num_mid, 1:num_mid) - fliplr(x_Q(1:num_mid, num_max+1-num_mid:num_max))...
       - flipud(x_Q(num_max+1-num_mid:num_max, 1:num_mid))...
       + rot90(x_Q(num_max+1-num_mid: num_max, num_max+1-num_mid: num_max),2);

if mod(num_max,2) == 1
   x_Q_odd(num_mid,:) = []; x_Q_odd(:,num_mid) = [];
end
