%% Calculating Energy of finite interaction in odd parity
clear
tic
load('equal_BA_inf.mat','root_ba')
% load('BAroots_finite_odd_100.mat', 'Bethe_roots')
% root_ba=Bethe_roots;
%% 定义参数
num=2500;        %要计算的根的数量 1300
root_inf=root_ba(1:num,:);
c_max=5*10^6;      %相互作用c的最大值需要估计，避免选的d的解离无穷大的解很远
c_min=100;         %你需要到达的最小值c  
...
%     Value(jj,1)=norm(fun_BAE_o(c_max,x0));  %验证无穷初值BA根  good  2e-04
%     if Value(jj)>0.001
...
%---------------用fsolve代替迭代，似乎可行？暂时没问题------------
        ...
        x1=fsolve(@(lam)fun_BAE_o(c,lam),x0,opt);
        x0=x1;
           if fun_BAE_o(c,x0)>10^(-6)
              %fsolve给的解不好则发出警告
...
En_odd2=sort(Bethe_roots(:,3));
%=================================
end_time=toc    %
save('equal_BA_finite_odd_c100.mat')                       
