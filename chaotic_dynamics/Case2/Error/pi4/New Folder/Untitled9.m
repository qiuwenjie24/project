...
num=5;        %要计算的根的数量
root_inf=root_ba;
c_max=10;      %相互作用c的最大值需要估计，避免选的d的解离无穷大的解很远
c_min=1;         %你需要到达的最小值c  
...
%     Value(jj,1)=norm(fun_BAE_e_initial(c_max,x0)/c_max);  %验证无穷初值BA根  good  2e-04
%     if Value(jj)>0.001
...
% %{
    ...
    c=c_max;
    while c-0.0001>c_min         %循环2 ---对不同相互作用循环，
        %根据相互作用范围大小来设置步长
        if c > 10^2
...
%---------------判断初值是否靠近0------------
%要求0.001*c_max*3/2 > 2*( abs(sqrt(3)*root_inf(num,1))+abs(3*root_inf(num,2)))/2
        if norm(fun_BAE_e_initial(c,x0))>0.01 
          %如果初值给得不够靠近方程的根(相差>1)则发出警告: 第几(j)个初值根有问题
           fun_BAE_e_initial(c,x0)/c
           fprintf('Initial value is too far--root jj=%g,c=%g, ------\n',jj,c)
           c=0;
           pause
...
%---------------用fsolve代替迭代，似乎可行？暂时没问题------------
        ...
        x1=fsolve(@(lam)fun_BAE_e_initial(c,lam)/c,x0,opt);
        x0=x1;
